...
void ddxReady(void) {
    ...
    if (NoListenAll)
        return;
    if (xstartup && !strlen(xstartup)) // allow overriding $LINUXGO_X11_XSTARTUP with empty xstartup arg
        return;
    if (!xstartup || !strlen(xstartup))
        xstartup = getenv("LINUXGO_X11_XSTARTUP");
    if (!xstartup || !strlen(xstartup))
        return;
...
Bool loriePresentFlip(__unused RRCrtcPtr crtc, __unused uint64_t event_id, __unused uint64_t target_msc, PixmapPtr pixmap, __unused Bool sync_flip) {
    ...
    char *forceFlip = getenv("LINUXGO_X11_FORCE_FLIP");
