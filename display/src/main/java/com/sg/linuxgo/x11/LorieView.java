...
public class LorieView extends SurfaceView implements InputStub {
    ...
    private final InputConnection mConnection = new InputConnectionWrapper(new BaseInputConnection(this, false) {
        ...
        private final Prefs mPrefs = a != null ? MainActivity.getPrefs() : new Prefs(getContext());
        ...
        boolean replaceText(CharSequence newText, boolean reuse) {
            ...
            if (a != null && a.useX11EKBarBehaviour && a.mExtraKeys != null)
    ...
    private Runnable mPendingResizeRunnable = null;
    ...
    private final SurfaceHolder.Callback mSurfaceCallback = new SurfaceHolder.Callback() {
        ...
        @Override public void surfaceChanged(@NonNull SurfaceHolder holder, int f, int width, int height) {
            ...
            final int finalWidth = getMeasuredWidth();
            final int finalHeight = getMeasuredHeight();

            Log.d("SurfaceChangedListener", "Surface was changed: " + finalWidth + "x" + finalHeight);
            if (mCallback == null)
                return;

            if (mPendingResizeRunnable != null) {
                mResizeHandler.removeCallbacks(mPendingResizeRunnable);
            }

            mPendingResizeRunnable = new Runnable() {
                @Override
                public void run() {
                    if (mCallback != null) {
                        getDimensionsFromSettings();
                        mCallback.changed(finalWidth, finalHeight, p.x, p.y);
                    }
                    mPendingResizeRunnable = null;
                }
            };
            mResizeHandler.postDelayed(mPendingResizeRunnable, 150); // 150ms debounce
        }
    ...
    void getDimensionsFromSettings() {
        ...
        Prefs prefs = a != null ? MainActivity.getPrefs() : new Prefs(getContext());
    ...
    public boolean dispatchKeyEventPreIme(KeyEvent event) {
        ...
        MainActivity a = MainActivity.getInstance();
        return a != null && a.handleKey(event);
    }
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        MainActivity a = MainActivity.getInstance();
        Prefs prefs = a != null ? MainActivity.getPrefs() : new Prefs(getContext());
        if (prefs.enforceCharBasedInput.get())
