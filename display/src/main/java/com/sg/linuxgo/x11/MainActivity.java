...
import static com.sg.linuxgo.x11.CmdEntryPoint.ACTION_START;
import static com.sg.linuxgo.x11.LoriePreferences.ACTION_PREFERENCES_CHANGED;
...
import com.sg.linuxgo.x11.input.InputEventSender;
import com.sg.linuxgo.x11.input.InputStub;
import com.sg.linuxgo.x11.input.TouchInputHandler;
import com.sg.linuxgo.x11.utils.FullscreenWorkaround;

import com.sg.linuxgo.x11.utils.SamsungDexUtils;
import com.sg.linuxgo.x11.utils.X11ExtraKeys;
import com.sg.linuxgo.x11.utils.X11ToolbarViewPager;
...
public class MainActivity extends AppCompatActivity {
    public static final String ACTION_STOP = "com.sg.linuxgo.x11.ACTION_STOP";
    public static final String ACTION_CUSTOM = "com.sg.linuxgo.x11.ACTION_CUSTOM";

    public static Handler handler = new Handler();
    FrameLayout frm;
    private TouchInputHandler mInputHandler;
    protected ICmdEntryInterface service = null;
    public X11ExtraKeys mExtraKeys;
    ...
    boolean useX11EKBarBehaviour = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.help_button).setOnClickListener((l) -> startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/termux/linuxgo-x11/blob/master/README.md#running-graphical-applications"))));
        ...
        mLorieKeyListener = (v, k, e) -> {
            ...
            if (useX11EKBarBehaviour && mExtraKeys != null && (dev == null || dev.isVirtual()))
    ...
    void onPreferencesChangedCallback() {
        ...
        useX11EKBarBehaviour = prefs.useX11EKBarBehaviour.get();
    ...
    private void setTerminalToolbarView() {
        ...
        layoutParams.height = Math.round(37.5f * getResources().getDisplayMetrics().density *
                (X11ExtraKeys.getExtraKeysInfo() == null ? 0 : X11ExtraKeys.getExtraKeysInfo().getMatrix().length));
    ...
    Notification buildNotification() {
        NotificationCompat.Builder builder =  new NotificationCompat.Builder(this, getNotificationChannel(mNotificationManager))
                .setContentTitle("PocketLinux X11")
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        // Edge-to-edge (API 35+): no setStatusBarColor / setNavigationBarColor,
        // no deprecated LAYOUT_IN_DISPLAY_CUTOUT_MODE_DEFAULT / SHORT_EDGES.
        ...
        if (hasFocus && SDK_INT >= VERSION_CODES.P) {
            final WindowManager.LayoutParams attrs = window.getAttributes();
            // hideCutout: keep content out of the notch. Otherwise draw edge-to-edge into cutout.
            if (hideCutout) {
                attrs.layoutInDisplayCutoutMode = LAYOUT_IN_DISPLAY_CUTOUT_MODE_NEVER;
            } else if (SDK_INT >= VERSION_CODES.R) {
                attrs.layoutInDisplayCutoutMode = LAYOUT_IN_DISPLAY_CUTOUT_MODE_ALWAYS;
            }
            // API 28–29: leave platform default (no SHORT_EDGES / DEFAULT constant usage).
            window.setAttributes(attrs);
        }

        window.addFlags(FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        // Drop legacy translucent / FLAG_FULLSCREEN; immersive uses WindowInsetsController.
        window.clearFlags(FLAG_FULLSCREEN | FLAG_TRANSLUCENT_STATUS);

        if (hasFocus) {
            final WindowInsetsControllerCompat insetsController =
                    new WindowInsetsControllerCompat(window, decorView);
            final int systemBars = WindowInsetsCompat.Type.statusBars()
        ...
        if (contentChild != null) {
            // Edge-to-edge: never use fitsSystemWindows; content draws behind bars when immersive.
    ...
    public void onUserLeaveHint() {
        // Never crash on leave-home (IllegalStateException if PiP disallowed / wrong state).
        if (!prefs.PIP.get() || !hasPipPermission(this)) return;
        if (isFinishing() || isChangingConfigurations()) return;
        if (isInPictureInPictureMode()) return;
        try {
            enterPictureInPictureMode();
        } catch (IllegalStateException e) {
            Log.w("MainActivity", "enterPictureInPictureMode: " + e.getMessage());
        } catch (Exception e) {
            Log.w("MainActivity", "enterPictureInPictureMode failed: " + e.getMessage());
        }
    }
