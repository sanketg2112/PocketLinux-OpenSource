...
/* Linked from libxtrans (Xtranssock.c) when UNIX_PATH uses TMPDIR */
...
Java_com_sg_linuxgo_x11_CmdEntryPoint_start(JNIEnv *env, __unused jclass cls, jobjectArray args) {
    pthread_t t;
    JavaVM* vm = NULL;
    ...
    if (getenv("LINUXGO_X11_DEBUG") && !fork()) {
    ...
    if (access("/data/data/com.sg.linuxgo/files/usr/lib/libtermux-exec.so", F_OK) == 0 && !detectTracer()
            && !getenv("XSTARTUP_LD_PRELOAD"))
        setenv("LD_PRELOAD", "/data/data/com.sg.linuxgo/files/usr/lib/libtermux-exec.so", 1);
    ...
    if (!getenv("TMPDIR")) {
        if (access("/tmp", F_OK) == 0)
            setenv("TMPDIR", "/tmp", 1);
        else if (access("/data/data/com.sg.linuxgo/files/usr/tmp", F_OK) == 0)
            setenv("TMPDIR", "/data/data/com.sg.linuxgo/files/usr/tmp", 1);
    }
...

    {
        ...
        /* Ensure the listen directory exists before dix_main binds X0 */
        mkdir(tmp, 0777);
        mkdir(xtrans_unix_dir_x11, 01777);
        chmod(xtrans_unix_dir_x11, 01777);
        log(INFO, "X11 unix socket path prefix: %s0  dir=%s",
            xtrans_unix_path_x11, xtrans_unix_dir_x11);
    }

    log(INFO, "Using TMPDIR=\"%s\"", getenv("TMPDIR"));
    ...
    if (!getenv("XKB_CONFIG_ROOT")) {
        ...
        // Termux case
        else if (access("/data/data/com.sg.linuxgo/files/usr/share/xkeyboard-config-2", F_OK) == 0)
            setenv("XKB_CONFIG_ROOT", "/data/data/com.sg.linuxgo/files/usr/share/xkeyboard-config-2", 1);
        else if (access("/data/data/com.sg.linuxgo/files/usr/share/X11/xkb", F_OK) == 0)
            setenv("XKB_CONFIG_ROOT", "/data/data/com.sg.linuxgo/files/usr/share/X11/xkb", 1);
    }
...
Java_com_sg_linuxgo_x11_CmdEntryPoint_getXConnection(JNIEnv *env, __unused jobject cls) {
    int client[2];
...
JNIEXPORT jobject JNICALL
Java_com_sg_linuxgo_x11_CmdEntryPoint_getLogcatOutput(JNIEnv *env, __unused jobject cls) {
    jclass ParcelFileDescriptorClass = (*env)->FindClass(env, "android/os/ParcelFileDescriptor");
    jmethodID adoptFd = (*env)->GetStaticMethodID(env, ParcelFileDescriptorClass, "adoptFd", "(I)Landroid/os/ParcelFileDescriptor;");
    const char *debug = getenv("LINUXGO_X11_DEBUG");
...
JNIEXPORT jboolean JNICALL
Java_com_sg_linuxgo_x11_CmdEntryPoint_connected(__unused JNIEnv *env, __unused jclass clazz) {
    return conn_fd != -1;
}

JNIEXPORT void JNICALL
Java_com_sg_linuxgo_x11_CmdEntryPoint_listenForConnections(JNIEnv *env, jobject thiz) {
