package com.sg.linuxgo.x11.utils;

import static com.sg.linuxgo.shared.extrakeys.ExtraKeysConstants.PRIMARY_KEY_CODES_FOR_STRINGS;
import static com.sg.linuxgo.x11.MainActivity.toggleKeyboardVisibility;
...
import com.sg.linuxgo.shared.extrakeys.*;
import com.sg.linuxgo.x11.LoriePreferences;
import com.sg.linuxgo.x11.MainActivity;
import com.sg.linuxgo.x11.Prefs;

import org.json.JSONException;

public class X11ExtraKeys implements ExtraKeysView.IExtraKeysView {
    @SuppressWarnings("FieldCanBeLocal")
    private static final String LOG_TAG = "X11ExtraKeys";
    ...
    public X11ExtraKeys(@NonNull View.OnKeyListener eventlistener, MainActivity activity, ExtraKeysView extrakeysview) {
    ...
    static final String ACTION_START_PREFERENCES_ACTIVITY = "com.sg.linuxgo.x11.start_preferences_activity";
    ...
    public static void setExtraKeys() {
        ...
        try {
            ...
            Prefs prefs = a != null ? MainActivity.getPrefs() : null;
            String extrakeys = prefs != null ? prefs.extra_keys_config.get() : X11ExtraKeys.DEFAULT_IVALUE_EXTRA_KEYS;
            mExtraKeysInfo = new ExtraKeysInfo(extrakeys, "extra-keys-style", ExtraKeysConstants.CONTROL_CHARS_ALIASES);
        } catch (JSONException e) {
            MainActivity a = MainActivity.getInstance();
            if (a != null)
                Toast.makeText(a, "Could not load and set the \"extra-keys\" property from the properties file: " + e, Toast.LENGTH_LONG).show();
            Log.e(LOG_TAG, "Could not load and set the \"extra-keys\" property from the properties file: ", e);

            try {
                mExtraKeysInfo = new ExtraKeysInfo(X11ExtraKeys.DEFAULT_IVALUE_EXTRA_KEYS, "default", ExtraKeysConstants.CONTROL_CHARS_ALIASES);
            } catch (JSONException e2) {
                if (a != null)
                    Toast.makeText(a, "Can't create default extra keys", Toast.LENGTH_LONG).show();
